...
class XMLFileNameMixin(object):
    def __init__(self, *args, **kwargs):
        ...
        identifiers = [
            ("thermo raw", "thermo"),
            ("abi wiff file", "wiff"),
            ("agilent masshunter format", "masshunter"),
        ]
        for filenode in file_info:
            ...
            file_params = dict(
                [
                    ...
                    for i in filetag.findall(
                        "{0}cvParam".format("{http://psi.hupo.org/ms/mzml}")
                    )
                ]
            )
...
class MZMLIterator(
    XMLFileNameMixin, templates.GenericIterator, GenericProteomicIterator
):
    def __init__(self, filename, full=False, store=True, ms_filter=False, start=0):
        ...
        if not self.ra or invalid_index:
            if invalid_index:
                print(
                    "mzML index contains duplicate values. Please rebuild index. Building a new index in memory."
                )
            else:
                print(
    ...
    def build_index(self, file_obj):
        ...
        while True:
            ...
            while i:
                ...
                scanId = self._get_scan_from_string(
                    cetree.fromstring(tag_header).get("id")
                )
        ...
        try:
            if spectra.tag == "{0}spectrum".format(namespace):
                ...
                spectra_params = dict(
                    [
                        (i.get("name"), i.get("value"))
                        for i in spectra.findall("{0}cvParam".format(namespace))
                    ]
                )
                scan_info = dict(
                    [
                        (i.get("name"), i.get("value"))
                        for i in spectra.findall(
                            "{0}scanList/{0}scan/{0}cvParam".format(namespace)
                        )
                    ]
                )
                precursor_info = dict(
                    [
                        (i.get("name"), i.get("value"))
                        for i in spectra.findall(
                            "{0}precursorList/{0}precursor/{0}selectedIonList/{0}selectedIon/{0}cvParam".format(
                                namespace
                            )
                        )
                    ]
                )
                parent_scan = spectra.findall(
                    "{0}precursorList/{0}precursor".format(namespace)
                )
                if parent_scan:
                    parent_scan = self._get_scan_from_string(
                        parent_scan[0].get("spectrumRef")
                    )
                ...
                if (not self.ms_filter or ms_level == self.ms_filter) and full:
                    mzmls, intensities = spectra.findall(
                        "{0}binaryDataArrayList/".format(namespace)
                    )
                    mzml_params = dict(
                        [
                            (i.get("name"), i.get("value"))
                            for i in mzmls.findall("{0}cvParam".format(namespace))
                        ]
                    )
                    mzmls = self.unpack_array(
                        mzmls.find("{0}binary".format(namespace)).text,
                        mzml_params,
                        namespace=namespace,
                    )
                    intensity_params = dict(
                        [
                            (i.get("name"), i.get("value"))
                            for i in intensities.findall("{0}cvParam".format(namespace))
                        ]
                    )
                    intensities = self.unpack_array(
                        intensities.find("{0}binary".format(namespace)).text,
                        intensity_params,
                        namespace=namespace,
                    )
                ...
                if self.filetype == "thermo":
                    target_info = dict(
                        [
                            (i.get("name"), i.get("value"))
                            for i in spectra.findall(
                                "{0}precursorList/{0}precursor/{0}isolationWindow/{0}cvParam".format(
                                    namespace
                                )
                            )
                        ]
                    )
                    scanObj.product_ion = target_info.get(
                        "isolation window target m/z", 0
                    )
                return scanObj
                ...
                if spectra.get("id") == "TIC":
                    ...
                    for info in spectra.findall(
                        "{0}binaryDataArrayList/".format(namespace)
                    ):
                        chroma_params = dict(
                            [
                                (i.get("name"), i.get("value"))
                                for i in info.findall("{0}cvParam".format(namespace))
                            ]
                        )
                        if "intensity array" in chroma_params:
                            intensities = self.unpack_array(
                                info.find("{0}binary".format(namespace)).text,
                                chroma_params,
                                namespace=namespace,
                            )
                        elif "time array" in chroma_params:
                            time_array = self.unpack_array(
                                info.find("{0}binary".format(namespace)).text,
                                chroma_params,
                                namespace=namespace,
                            )
                    ...
                    precursor_info = dict(
                        [
                            (i.get("name"), i.get("value"))
                            for i in spectra.findall(
                                "{0}precursor/{0}isolationWindow/{0}cvParam".format(
                                    namespace
                                )
                            )
                        ]
                    ...
                    product_info = dict(
                        [
                            (i.get("name"), i.get("value"))
                            for i in spectra.findall(
                                "{0}product/{0}isolationWindow/{0}cvParam".format(
                                    namespace
                                )
                            )
                        ]
                    )
                    scanObj.product_ion = float(
                        product_info.get("isolation window target m/z", 0)
                    )
                    # if self.filetype == 'wiff':
                    #     pass
                    # else:
                    ...
                    if (not self.ms_filter or ms_level == self.ms_filter) and full:
                        ...
                        intensity_params = dict(
                            [
                                (i.get("name"), i.get("value"))
                                for i in intensities.findall(
                                    "{0}cvParam".format(namespace)
                                )
                            ]
                        )
                        intensities = self.unpack_array(
            ...
            sys.stderr.write(
                "Error unpacking spectra {}:\n {}".format(
                    spectra, traceback.format_exc()
                )
            )
            return None
            # elif spect:
    ...
    def writeScans(
        self, handle=None, scans=None, namespace="{http://psi.hupo.org/ms/mzml}"
    ):
        ...
        for scan_index, scan_offset_info in enumerate(scan_indices):
            offset = etree.SubElement(
                spectrumIndex, "offset", {"idRef": scan_offset_info["idRef"]}
            )
            ...
            if self.gzip:
                ...
                while entry:
                    matching_tags = list(
                        filter(
                            lambda x: x[0] != -1,
                            ((entry.find(i), i) for i in opening_tags),
                        )
                    )
                    if any(matching_tags):
                        opening_tag = sorted(matching_tags, key=operator.itemgetter(0))[
                            0
                        ][1][1:].strip()
                        break
                    entry = maybe_decode(self.handle.readline())
...
class PepXMLIterator(
    XMLFileNameMixin, GenericProteomicIterator, templates.GenericIterator
):
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    try:
                        self.mzml = MZMLIterator(
                            "%s%s.gz" % (new_path, info["raw_data"])
                        )
                    except IOError:
                        self.mzml = None
            self.handle.seek(0)
            ...
            self.lxml = False
            sys.stderr.write(
                "pepXML parsing unavailable: lxml is required to parse mzML files\n"
            )
            return
        if self.lxml:
            self.spectra = dom1
        ...
        if spectra.tag == "{0}spectrum_query".format(namespace):
            ...
            for i in analysis:
                ...
                if analysis_type == "peptideprophet":
                    ...
                    probs = results.findall(
                        "{0}mod_aminoacid_probability".format(namespace)
                    )
                    ...
                    for prob in probs:
                        ...
                        try:
                            ptmprophet_info[mod_pos][mod_mass] = prob_info[
                                "probability"
                            ]
                        except KeyError:
                            ptmprophet_info[mod_pos] = {
                                mod_mass: prob_info["probability"]
                            }
            if iprophet_expect:
            ...
            alt_proteins = search_result.findall(
                ".//{0}alternative_protein".format(namespace)
            )
            ...
            if ptmprophet_info:
                for position, modification in ptmprophet_info.iteritems():
                    ...
                    for mass, prob in modification.iteritems():
                        pepObj.addModification(
                            mod_aa, position, mass, "%s(%s)" % (mass, prob)
                        )
            else:
                mods = search_result.findall(
                    ".//{0}mod_aminoacid_mass".format(namespace)
                )
                for i in mods:
                    ...
                    pepObj.addModification(
                        mod_aa, mod_pos, mod_info["mass"], mod_info["mass"]
                    )
...
class mzDataIterator(templates.GenericIterator, GenericProteomicIterator):
    ...
    def __init__(self, filename, **kwargs):
        ...
        try:
            ...
            for tag, peptideXML in peps:
                ...
                userParams = dict(
                    [
                        ...
                        for j in additional.findall("userParam")
                    ]
                )
                cvParam = dict(
                    [
                        (j.get("name"), j.get("value"))
                        for j in additional.findall("cvParam")
                    ]
                )
            ...
            self.lxml = False
            sys.stderr.write(
                "mzData parsing unavailable: lxml is required to parse mzData files\n"
            )
            return
        if self.lxml:
            self.spectra = dom1
    ...
    def unpack_array(self, array, params, namespace="{http://psi.hupo.org/ms/mzml}"):
        ...
        if params.get("precision", False) == "64":
            array = [
                struct.unpack("d", array[i : i + 8])[0]
                for i in six.moves.range(0, len(array), 8)
            ]
        else:
            array = [
                struct.unpack("f", array[i : i + 4])[0]
                for i in six.moves.range(0, len(array), 4)
            ]
        return array
    ...
    def parselxml(self, spectra, full=False, namespace=""):
        ...
        precursorInfo = spectra.iterfind(
            "spectrumDesc/precursorList/precursor/ionSelection"
        )
        for precursor in precursorInfo:
            cvParams = dict(
                [(j.get("name"), j.get("value")) for j in precursor.findall("cvParam")]
            )
        ...
        scanObj.scans = sorted(
            [(i, j) for i, j in zip(mzData, intData)], key=operator.itemgetter(0)
        )
        if self.store:
            ...
            if "residue, modification mass" in label:
                ...
                if silac_matches:
                    for silac_match in silac_matches:
                        aa, mass = (
                            silac_match.group("residue"),
                            silac_match.group("mass"),
                        )
    ...
    def parseModel(self, element):
            ...
            if i.attrib["label"] == "fragment ion mass spectrum":
                ...
                for j in ab:
                    ...
                    for k in mzIter:
                        mz = [
                            mval
                            for mval in k.text.strip().replace("\n", " ").split(" ")
                        ]
        ...
        for peptide_index, protein in enumerate(proteins):
            ...
            if added is False:
                ...
                peptide = domain.attrib["seq"].replace(
                    " ", ""
                )  # for some reason it can have spaces
                pExpect = domain.attrib["expect"]
                scanObj.expect = float(pExpect)
                for mod in mods:
                    scanObj.addModification(
                        mod.attrib["type"],
                        int(mod.attrib["at"]) - start,
                        float(mod.attrib["modified"]),
                        mod.attrib["modified"],
                    )
    ...
    def openMGFIndex(self, path):
        try:
            ...
            if os.path.exists(path):
                raise Exception(
                    "Index file path: %s found, but appears incomplete" % path
                )
            f = open(path, "wb")
            while True:
                try:
...
class MascotDATIterator(templates.GenericIterator, GenericProteomicIterator):
    def __init__(self, filename):
        ...
        results = msparser.ms_peptidesummary(
            resfile,
            msparser.ms_mascotresults.MSRES_MAXHITS_OVERRIDES_MINPROB,
            0,
            0,
            "",
            0,
        ...
        self.results = msparser.ms_peptidesummary(
            self.resfile,
        ...
        if self.sparams.getQUANTITATION():
            ...
            self.quantFile.setSchemaFileName(
                "".join(
                    [
            ...
            if self.resfile.getQuantitation(self.quantFile):
                if not self.quantFile.isValid():  # true if errors
                    ...
                    self.quantMethod = self.quantFile.getMethodByName(
                        self.sparams.getQUANTITATION()
                    )
                    if self.quantMethod:
        ...
        self.modFile = msparser.ms_modfile(
            self.uModFile, msparser.ms_umod_configfile.MODFILE_FLAGS_ALL
        )
        ...
        vInd = 1
        modText = self.resfile.getSectionValueStr(
            msparser.ms_mascotresfile.SEC_MASSES, "delta%d" % vInd
        )
        while modText:
        ...
        vInd = 1
        modText = self.resfile.getSectionValueStr(
            msparser.ms_mascotresfile.SEC_MASSES, "FixedMod%d" % vInd
        )
        while modText:
            modMass, modName = modText.split(",")
            objMod = self.modFile.getModificationByName(modName)
        ...
        seci = 1
        seckey = self.resfile.enumerateSectionKeys(
            msparser.ms_mascotresfile.SEC_MASSES, seci
        )
        self.groupMasses = {}
        ...
        while seckey:
            ...
            if secMatch:
                ...
                self.vmMass[secMatchInd] = self.resfile.getSectionValueStr(
                    msparser.ms_mascotresfile.SEC_MASSES, seckey
                ).split(",")
                ...
                secval = self.resfile.getSectionValueStr(
                    msparser.ms_mascotresfile.SEC_MASSES, seckey
                )
        ...
        modind = 1
        mod = self.resfile.getSectionValueStr(
            msparser.ms_mascotresfile.SEC_MASSES, "delta%d" % modind
        )
        while mod:
            modind += 1
            mod = self.resfile.getSectionValueStr(
        ...
        self.cRules = set(
            [
                msparser.ms_fragmentationrules.FRAG_IMMONIUM,
                msparser.ms_fragmentationrules.FRAG_INTERNAL_YA,
                msparser.ms_fragmentationrules.FRAG_INTERNAL_YB,
            ]
        )
        self.fragRuleMapping = {
            msparser.ms_fragmentationrules.FRAG_IMMONIUM: "immonium ion",
                ...
                if p != -1 and query != -1:
                    ...
                    for aanum, residue in enumerate(vmods):
                        ...
                        if residue:
                            ...
                            scanObj.addModification(
                                peptide[aanum], pstart - 1 + aanum, modMass, modName
                            )
                    ...
                    if full:
                        for ionIndex in six.moves.range(1, 4):
                            for peakIndex in six.moves.range(
                                inputQuery.getNumberOfPeaks(ionIndex)
                            ):
                                scanObj.scans.append(
                                    (
                                        inputQuery.getPeakMass(ionIndex, peakIndex),
                                        float(
                                            inputQuery.getPeakIntensity(
                                                ionIndex, peakIndex
                                            )
                                        ),
                                    )
                        ...
                        rules = [
                            int(ruleNum)
                            for ruleNum in self.sparams.getRULES().split(",")
                        ]
                        rules.sort()
                        chargeRule = 0
                        matched = {
                        ...
                        for rule in (int(arule) for arule in rules):
                            if rule == 1:
                                ...
                                for chargeState in six.moves.range(charge + 1):
                                    if (
                                        (chargeState == 1 and chargeRule == 1)
                                        or (chargeState == 2 and chargeRule == 2)
                                        and rule not in self.cRules
                                    ):
                                        frags = msparser.ms_fragmentvector()
                                        errs = msparser.ms_errs()
                                        self.aahelper.setMasses(self.massFile)
                                        self.aahelper.setAvailableModifications(
                                            self.vecFixed, self.vMods
                                        )
                                        fres = self.aahelper.calcFragmentsEx(
                                            pep,
                                            rule,
                                            chargeState,
                                            0,
                                            pep.getMrCalc(),
                                            self.sparams.getMassType(),
                                            frags,
                                            errs,
                                        )
                                        if not fres:
                                            ...
                                            frags.addExperimentalData(
                                                self.resfile, query
                                            )
                                            for frag in (
                                                frags.getFragmentByNumber(fragIndex)
                                                for fragIndex in six.moves.range(
                                                    frags.getNumberOfFragments()
                                                )
                                            ):
...
class ThermoMSFIterator(templates.GenericIterator, GenericProteomicIterator):
    def __init__(
        self, filename, clvl=3, srank=1, full=False, store=False, peptide=None
    ):
        self.clvl = clvl
        self.full = full
        self.srank = srank
        if isinstance(filename, six.string_types):
            ...
            raise Exception(
                TypeError, "Unknown Type of filename -- must be a file path"
            )
        ...
        self.aaTable = dict(
            [
                ...
                for i in self.conn.execute(
                    "select aa.AminoAcidID, aa.OneLetterCode from aminoacids aa"
                ).fetchall()
            ]
        )
        ...
        if peptide is not None and not isinstance(peptide, (tuple, set, list)):
            peptide = [peptide]
        self.extra = (
            "and p.Sequence IN ({})".format(
                ",".join(['"{}"'.format(i) for i in peptide])
            )
            if peptide is not None
            else ""
        )
        try:
            sql = "select COUNT(distinct p.SpectrumID) from peptides p where p.PeptideID IS NOT NULL and p.ConfidenceLevel >= {} and p.SearchEngineRank <= {} {}".format(
                clvl, srank, self.extra
            )
            ...
            sql = "select COUNT(distinct p.SpectrumID) from peptides p where p.PeptideID IS NOT NULL and p.ConfidenceLevel >= {} {}".format(
                clvl, self.extra
            )
            self.nrows = self.conn.execute(sql).fetchone()[0]
    ...
    def getSILACLabels(self):
        ...
        if self.version == 1:
            ...
            silac = etree.fromstring(
                [i for i in self.root.iterdescendants("QuantitationMethod")][
                    0
                ].text.encode("utf-16")
            )
        if silac is None:
        ...
        for method in silac.findall("*MethodPart"):
            ...
            masses = {}
            for mod_info in [
                etree.fromstring(html_parser.unescape(i.text))
                for i in method.iter("Parameter")
                if "Modification Version" in i.text
            ]:
    ...
    def loadChromatogram(self):
        ...
        for chroma in self.cur.fetchall():
            ...
            for j in zf.namelist():
                ...
                chromObj = Chromatogram()
                chromObj.times, chromObj.intensities = zip(
                    *[
                        (float(point.attrib["T"]), float(point.attrib["Y"]))
                        for point in xml.findall("Points/*")
                    ]
                )
                if "TicTrace" in j:
                    self.chromatogram = chromObj
    ...
    def getScan(self, specId, peptide=None):
        ...
        get a random scan
        """
        sql = self.base_sql + " where sh.SpectrumID = %d and p.Sequence = '%s'" % (
            int(specId),
            peptide,
        )
        self.cur.execute(sql)
    ...
    def getScans(self, modifications=False, fdr=True):
        ...
        if fdr:
            sql = (
                self.base_sql
                + "WHERE p.ConfidenceLevel >= {} and p.SearchEngineRank <= {} {}".format(
                    self.clvl, self.srank, self.extra
                )
            )
            try:
                self.cur.execute(sql)
            except sqlite3.OperationalError:
                sql = self.base_sql + "WHERE p.ConfidenceLevel >= {} {}".format(
                    self.clvl, self.extra
                )
                self.cur.execute(sql)
        else:
            sql = self.base_sql
            ...
            for row in msStr:
                if stage == 0:
                    ...
                    if "PrecursorInfo" in row:
                        ...
                        if not float(smass):
                            smass = (
                                float(finfo[5]) + (int(charge) - 1) * config.PROTON
                            ) / float(charge)
                        scanObj.charge = charge
    ...
    def parseScan(self, i, modifications=True, full=False):
        ...
        for confidence, searchRank, sequence, pepId in zip(
            i[0].split(","), i[1].split(","), i[2].split(","), i[3].split(",")
        ):
    ...
    def parseFullScan(self, i, modifications=False):
        ...
        if modifications:
            sql = (
                "select aam.ModificationName,pam.Position,aam.DeltaMass from peptidesaminoacidmodifications pam left join aminoacidmodifications aam on (aam.AminoAcidModificationID=pam.AminoAcidModificationID) where pam.PeptideID=%s"
                % pid
            )
            for row in self.conn.execute(sql):
                scanObj.addModification(
                    peptide[row[1]], str(row[1]), str(row[2]), row[0]
                )
        else:
            ...
            if mods is not None:
                for modId, modPosition in zip(mods[0].split(","), mods[1].split(",")):
                    modEntry = self.modTable[str(modId)]
                    scanObj.addModification(
                        peptide[int(modPosition)], modPosition, modEntry[1], modEntry[0]
                    )
...
class MQIterator(templates.GenericIterator, GenericProteomicIterator):
    ...
    def getScan(self, specId, peptide=None):
        ...
        """
        sql = (
            "select sp.Spectrum, p.Sequence, p.PeptideID from spectrumheaders sh left join spectra sp on (sp.UniqueSpectrumID=sh.UniqueSpectrumID) left join peptides p on (sh.SpectrumID=p.SpectrumID) where sh.SpectrumID = %d and p.Sequence = '%s'"
            % (int(specId), peptide)
        )
        self.cur.execute(sql)
        i = self.cur.fetchone()
    ...
    def getScans(self, modifications=True, fdr=True):
        ...
        if fdr:
            sql = (
                "select sp.Spectrum, p.Sequence, p.PeptideID, p.SpectrumID from spectrumheaders sh left join spectra sp on (sp.UniqueSpectrumID=sh.UniqueSpectrumID) left join peptides p on (sh.SpectrumID=p.SpectrumID) WHERE p.ConfidenceLevel >= %d and p.SearchEngineRank <= %d"
                % (self.clvl, self.srank)
            )
            try:
                self.cur.execute(sql)
            except sqlite3.OperationalError:
                sql = (
                    "select sp.Spectrum, p.Sequence, p.PeptideID, p.SpectrumID from spectrumheaders sh left join spectra sp on (sp.UniqueSpectrumID=sh.UniqueSpectrumID) left join peptides p on (sh.SpectrumID=p.SpectrumID) WHERE p.ConfidenceLevel >= %d"
                    % self.clvl
                )
                self.cur.execute(sql)
        else:
    ...
    def getPeptide(self, peptide=None):
        sql = (
            "select sp.Spectrum, p.Sequence, p.PeptideID from spectrumheaders sh left join spectra sp on (sp.UniqueSpectrumID=sh.UniqueSpectrumID) left join peptides p on (sh.SpectrumID=p.SpectrumID) where p.Sequence = '%s'"
            % (peptide)
        )
        self.cur.execute(sql)
        for i in self.cur.fetchall():
