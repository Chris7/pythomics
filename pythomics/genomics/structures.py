        ...
        info = "\n".join(
            [
                "##INFO=<ID={0},Number={1},Type={2},Description={3}>".format(
                    key,
                    val.get("num_entries", "."),
                    type_map.get(val.get("type", "")),
                    val.get("description"),
                )
                for key, val in self.meta.info.iteritems()
            ]
        )
        filter = "\n".join(
            [
                "##FILTER=<ID={0},Description={1}>".format(
                    key, val.get("description", ".")
                )
                for key, val in self.meta.filter.iteritems()
            ]
        )
        format = "\n".join(
            [
                "##FORMAT=<ID={0},Number={1},Type={2},Description={3}>".format(
                    key,
                    val.get("num_entries", "."),
                    type_map.get(val.get("type", "")),
                    val.get("description"),
                )
                for key, val in self.meta.format.iteritems()
            ]
        )
        alt = "\n".join(
            [
                "##ALT=<ID={0},Description={1}>".format(
                    key, val.get("description", ".")
                )
                for key, val in self.meta.alt.iteritems()
            ]
        )
        header = "\t".join(
...
class VCFEntry(object):
    ...
    def __str__(self):
        ...
        return "\t".join(
            [
                ...
                "." if not self.alt else ",".join(self.alt),
                self.qual,
            ...
            if index != -1:
                ...
                if individual == -1:
                    if len(d) != len(self.samples):
                        [
                            self.parse_sample(i)
                            for i in six.moves.range(len(self.samples))
                        ]
    ...
    def is_homozygous(self, individual=None):
        ...
        if individual is not None:
            ...
            return [
                sum(alts) == len(alts) if sum(alts) > 0 else False
                for i, alts in self.genotype.iteritems()
            ]
    ...
    def get_alt(self, individual=0, nucleotides_only=True):
        ...
        if nucleotides_only:
            return [
                self.alt[i - 1].replace(".", "")
                for i in self.genotype[individual]
                if i > 0 and not self.alt[i - 1].startswith("<")
            ]
        else:
            return [
                self.alt[i - 1].replace(".", "")
        ...
        for info_n, sample_info in enumerate(sample.split(":")):
            if info_n == self.GT:
                if len(sample_info) == 3 and ("|" in sample_info or "/" in sample_info):
                    self.genotype[individual] = [
                        int(i) for i in GENOTYPE_SPLIT.split(sample_info)
                    ]
            elif info_n == self.GQ:
                if not sample_info:
...
class GFFObject(SequenceObject):
    ...
    def __str__(self):
        attributes = self.info_delimiter.join(
            [
                "%s%s%s%s%s"
                % (key, self.key_delimiter, self.quotechar, value, self.quotechar)
