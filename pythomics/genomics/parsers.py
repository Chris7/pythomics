    ...
    def __init__(self, filename, sample=None):
        ...
        while row[:2] == "##":
            if row.startswith("##INFO="):
                assert self.vcf_file.add_info(row) is True
...
class GFFReader(templates.GenericIterator):
    def __init__(
        ...
        if preset and preset.lower() == "cufflinks":
            ...
            self.parent_tag = (
                "gene_id"  # 'transcript_id'#it's weird, but we're our own parent
            )
        else:
        ...
        for gff_object in GFFIterator(
            ...
            fast_lookup = [
                fast_access
                for fast_access in self.filters
                for attribute in gff_object.attributes
                if fast_access in attribute
            ]
        ...
        for child_gff_object in child_list:
            ...
            for parent_id in parent_ids:
                ...
                parent_feature.add_child(
                    self.feature_map.get(
                        child_gff_object.attributes.get(self.id_tag, None)
                    )
                )
    ...
    def get_attribute(self, attribute, value=None, features=False):
        ...
        if attribute in self.filters:
            valid_gff_objects = (
                self.fast_attributes[attribute]
                if not value
                else [
                    i
                    for i in self.fast_attributes[attribute]
                    if i.attributes.get(attribute, False) == value
                ]
            )
            if features:
                valid_ids = [
                    gff_object.attributes.get(self.id_tag, None)
                    for gff_object in valid_gff_objects
                ]
            ...
            valid_gff_objects = [
                gff_object
                for gff_feature in self.feature_map.values()
...
class VCFReader(templates.GenericIterator, VCFMixin):
    def __init__(
        self, filename, append_chromosome=False, store_positions=True, sample=None
    ):
        ...
        for vcf_entry in self.vcf_iterator:
            ...
            if store_positions:
                ...
                furthest_end = max(
                    [
                        abs(i) if i is not None else None
                        for i in vcf_entry.get_alt_lengths()
                    ]
                )
