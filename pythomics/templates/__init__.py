...
class CustomParser(argparse.ArgumentParser):
    ...
    def add_enzyme(self, help="The enzyme to cleave with."):
        self.add_argument(
            ...
            help=help,
            nargs="+",
            choices=list(protein_config.ENZYMES.keys()),
            type=str,
            default="trypsin",
        )
        self.add_argument(
            "--enzyme-pattern",
            help="A regex cleavage pattern such as [KR]|{P} to cleave proteins with.",
            type=str,
        )
    ...
    def add_delimited_file(
        ...
        self.add_argument(
            *delimiter, help="The delimiter for fields.", type=str, default="\t"
        )
        self.add_argument(
            *cols,
            help="The column of interest (default: 1). Can be a column name.",
            type=str,
            default=col_default
        )
        self.add_argument(
            *header,
            help="The number of headers lines (default: 1).",
            type=int,
            default=1
        )
    ...
    def add_read_pair(self):
        self.add_argument(
            "--left",
            help="The left (5') read pairs",
            nargs="?",
            type=argparse.FileType("r"),
        )
        self.add_argument(
            "--right",
            help="The right (3') read pairs",
            nargs="?",
            type=argparse.FileType("r"),
        )
    ...
    def add_vcf(self, help="The VCF file to use."):
        ...
        self.add_argument(
            "--no-homozygous",
            help="Don't include homozygous variants (default to include)",
            action="store_false",
        )
        self.add_argument(
            "--heterozygous", help="Use heterozygous variants", action="store_false"
        )
        self.add_argument(
            "--no-snps", help="Don't use SNPs (default to true).", action="store_false"
        )
        self.add_argument("--dels", help="Use Deletions.", action="store_true")
        self.add_argument("--ins", help="Use Insertions.", action="store_true")
        self.add_argument(
            "--individual", help="The Individual to use.", type=str, default=None
        )
        self.add_argument(
            "--append-chromosome",
            help="Should 'chr' be appended to the chromosome column?.",
            action="store_true",
        )
    ...
    def add_gff(self):
        ...
        gff_group.add_argument(
            "--gff",
            help="The GFF file to use.",
            type=argparse.FileType("r"),
            required=True,
        )
        gff_group.add_argument(
            "--group-on",
            help="The key to group entries together by (such as transcript_id)",
            type=str,
            default="ID",
        )
        gff_group.add_argument(
            "--feature",
            help="The feature to use for fetching coordinates (such as CDS, does not apply with cufflinks flag).",
            type=str,
            default="",
        )
        gff_group.add_argument(
            "--cufflinks",
            help="If the gff file is in the standard cufflinks output",
            action="store_true",
        )
        return gff_group
    ...
    def add_ms_files(
        self, help="The corresponding raw mass spectrometry file (ie .raw, .mzml)."
    ):
        self.add_argument("--raw", nargs="?", help=help, type=str)

    def add_processed_ms(
        self,
        group=None,
        help="The corresponding processed mass spectrometry files (ie .msf, .dat).",
        required=True,
    ):
        parser = self if group is None else group
        parser.add_argument(
            "--processed", help=help, type=argparse.FileType("r"), required=required
        )

    def add_column_function(
        self,
        argument,
        help="The column to apply a function to (if you want to combine fields, sum fields, etc.).",
        col_argument=None,
        group=None,
        col_help="The function to apply.",
        col_default="concat",
        parent=True,
    ):
        adder = self
        ...
        adder.add_argument(
            col_argument,
            help="{0} Options: {1}".format(
                col_help, ", ".join(ColumnFunctions.METHODS)
            ),
            type=str,
            choices=ColumnFunctions.METHODS,
            default=col_default,
        )
