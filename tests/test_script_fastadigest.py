import os, unittest, hashlib, subprocess, sys
...
class Test_Script_Fasta_Digest(unittest.TestCase):
    ...
    def test_fasta_genome_digest_no_enzyme(self):
        job = subprocess.Popen([sys.executable, os.path.join(self.script_dir, 'fastadigest.py'), '-f', self.nucleotide_file, '--genome', '--min', '0',
    ...
    def test_fasta_genome_digest_trypsin(self):
        job = subprocess.Popen([sys.executable, os.path.join(self.script_dir, 'fastadigest.py'), '-f', self.nucleotide_file, '--genome', '--min', '6',
    ...
    def test_fasta_protein_digest_no_min_length(self):
        job = subprocess.Popen([sys.executable, os.path.join(self.script_dir, 'fastadigest.py'), '-f', self.iterator_file, '--min', '0',
    ...
    def test_fasta_protein_digest_trypsin(self):
        job = subprocess.Popen([sys.executable, os.path.join(self.script_dir, 'fastadigest.py'), '-f', self.iterator_file, '--min', '6',
