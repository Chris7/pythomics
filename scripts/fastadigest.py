...
parser.add_argument(
    "--genome",
    help="Are we translating a genome? This will keep chromosome positions in the header.",
    action="store_true",
)
parser.add_out()
parser.add_enzyme()
parser.add_argument(
    "--unique", help="Only return unique peptides per cleavage", action="store_true"
)
            ...
            for header, sequence in fasta_file:
                ...
                for i in six.moves.range(digest_frame):
                    ...
                    if genome:
                        ...
                        translation = [
                            "".join(j)
                            for j in six.moves.zip_longest(
                                translation[0::2], translation[1::2], fillvalue=""
                            )
                        ]
                    else:
