...
group.add_argument(
    "--inference",
    help="The protein inference file (your peptide file with gene/protein annotations). For multiple files, separate by spaces (must be in same order as mods).",
    nargs="+",
...
group.add_argument(
    "--protein", help="The Protein column name", type=str, default="Protein"
)
group.add_argument(
    "--peptide", help="The Peptide column name", type=str, default="Peptide"
)
group.add_argument(
    "--quant",
    help="The name of quantification columns (such as Heavy/Light). Separate multiple columns by spaces",
    nargs="+",
    default=["Heavy/Light"],
)
...
mods.add_argument(
    "--mods",
    help="The modifications file (the file with sites, peptides). For multiple files, separate by spaces (must be in same order as inference).",
    nargs="+",
    type=argparse.FileType("r"),
    required=True,
)
...
parser.add_argument(
    "--no-median",
    help="Do not normalize quantification values by the median of the experiment.",
    action="store_true",
)
parser.add_argument(
    "--wp",
    help="The whole proteome inference file, if it exists. For multiple replicates, separate by spaces.",
    nargs="+",
    type=argparse.FileType("r"),
)
parser.add_argument(
...
parser.add_argument(
    "--site-file",
    help="The output path for the file with sumamries at the site level.",
    default=sys.stdout,
    type=argparse.FileType("wb"),
)
parser.add_argument(
    "--peptide-file",
    help="The output path for the file with sumamries at the site and peptide level.",
    default=sys.stdout,
    type=argparse.FileType("wb"),
)
...
def main():
    ...
    wp_summary = pd.DataFrame(
        columns=quant_cols + ["Mean"],
        index=set([i for wp in wp_reps for i in wp.index]),
    )
    for quant_col in quant_cols:
        wp_summary[quant_col] = (
            pd.concat(
                [wp[(wp_index, quant_col)] for wp_index, wp in enumerate(wp_reps)],
                axis=1,
            ).median(axis=1)
            if wp_reps
            else 0
    ...
    def process_file(
        ...
        for index, row in mods.iterrows():
            ...
            for peptide in peptides:
                genes = (
                    all_data[(all_data[pep_col] == peptide) == True][gene_col]
                    .str.split(";")
                    .fillna("NA")
                )
                gene = ";".join(
                    list(OrderedDict.fromkeys([j for i in genes for j in i]))
                )
                ...
                for quant_col in file_quant_cols:
                    med = (
                        (
                            np.log2(matches[quant_col])
                            if log_norm
                            else matches[quant_col]
                        )
                        - wp_median
                    ).median()
    ...
    for ptm_file, mod_file in zip(inference_files, mod_files):
        ...
        process_file(
            ...
            all_data=p1,
            col_names=colnames,
            file_quant_cols=quant_cols,
            wp_median=wp_medians,
            df=df,
            df_collapsed=df_collapsed,
        )
        if non_mod_norm:
            a = (
                p1[p1["Modifications"].str.contains("Phospho") == False]
                .groupby((gene_col, prot_col, pep_col))
                .median()
            )
    ...
    for i, v in df_collapsed.iteritems():
        for col in v.keys():
            collapsed_items = (
                [k for k in v[col] if not pd.isnull(k)]
                if isinstance(v[col], set)
                else v[col]
            )
    ...
    for quant_col in quant_cols:
        df["{}_Medians".format(quant_col)] = df.loc[:, colnames[quant_col]].median(
            axis=1
        )
        df_collapsed["{}_Medians".format(quant_col)] = df_collapsed.loc[
            :, colnames[quant_col]
        ].median(axis=1)
