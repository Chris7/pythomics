...
def main():
    ...
    for feature_name, feature in gff.get_features():
        ...
        if len(children) > 1:
            starts = dict(
                [(j.start, j) for i, v in children.iteritems() for j in v.parts()]
            )
            if len(starts) > 1:
                ...
                for ri, read in enumerate(parts[:-1]):
                    ...
                    try:
                        for i in samfile.fetch(read[0], int(read[2]) - 1, read[2]):
                            if (
                                not i.overlap(int(read[2]) - 1, int(read[2]))
                                or i.qname in written
                            ):
                                continue
                            reads.add(i.qname)
                            read_dict[i.qname] = i
                        ...
                        for i in samfile.fetch(read2[0], read2[1], int(read2[1]) + 1):
                            if (
                                not i.overlap(int(read2[2]) - 1, int(read2[2]))
                                or i.qname in written
                            ):
                                continue
                            reads2.add(i.qname)
