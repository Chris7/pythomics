...
parser.add_argument(
    "--substring",
    help="If set, merge features by partial matches (such as collapsing peptides into larger peptides)",
    action="store_true",
)
...
parser.add_argument(
    ...
    help="If the summary column has a delimiter, such as a ; for multiple proteins.",
)
parser.add_argument(
    "--strict",
...
parser.add_argument(
    "--merge", help="Merge together identical entries.", action="store_true"
)
...
def main():
    ...
    for peptide in reversed(peptide_keys):
        ...
        if first_match > 0:
            if peptide_join:
                matching_peptide = peptide_keys[
                    peptide_string.count("\n", 0, first_match)
                ]
            else:
                ...
                for i, v in enumerate(entry):
                    if i == mod_col:
                        if summary_col_delim:
                            result = [j for i in v for j in i.split(summary_col_delim)]
                        else:
                            result = v
                        entry_string.append(
                            mod_col_func(set(result) if merge else result)
                        )
                    else:
