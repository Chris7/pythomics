...
parser.add_out()
parser.add_enzyme(
    help='Enzyme to use. Pass a list like "trypsin lysc" to use multiple enzymes.  '
                    ...
                    for peptide in set(
                        ...
                        if len(peptide) > digest_max:
                            ...
                            try:
                                ...
                                coverageMap[header] = set([peptide])
        sys.stderr.write(
            "%d total peptides after digesting with %s\n"
    ...
    with args.out as o:
        ...
        for peptide in peptides_found:
            ...
            for header in peptides_found[peptide]:
                ...
                for match_position in sites:
                    found -= set(
                        six.moves.range(match_position, match_position + len(peptide))
                    )
        ...
        for header in coverageMap:
            ...
            if header in found_proteins:
                detected += 1
        sys.stderr.write(
            "average coverage is %0.4f over entire proteome\n"
            % (float(tlen - missed_len) / float(tlen))
        )
        sys.stderr.write(
            "average coverage is %0.4f over detected proteins\n" % (avg_cov / detected)
        )
