...
parser.add_argument(
    "--strict",
    help="For numeric operations, fail if types are incorrect (converting NA to a float for instance).",
    action="store_true",
)
...
parser.add_argument(
    "-r",
    "--regex",
    help="A perl regular expression determining which parts of the header to capture.",
    type=str,
)
parser.add_argument(
    "--inferred-name",
    help="The name you want to assign for protein inference (in case you are regexing for gene names or something).",
    type=str,
    default="Proteins",
)
parser.add_argument(
...
ibaq_group.add_argument(
    "--precursors",
    help="The column with precursor area (defaults to header lines containing 'Precursor').",
    type=str,
)
parser.add_column_function(
    ...
    group=ibaq_group,
    col_help="The function to apply to groups of iBAQ values (for multiple peptide matches).",
    parent=False,
)
ibaq_group.add_argument(
    "--non-redundant",
    help="Use only non-redundant theoretical tryptic peptides for the iBAQ denominator.",
    action="store_true",
)
parser.add_enzyme(help="The enzyme used to digest the sample.")
ibaq_group.add_argument(
    "--normalize",
    help="Normalize iBAQ to total intensity of column (useful for comparing multiple samples).",
    action="store_true",
)
...
protein_group.add_argument(
    "--position", help="Write the position of the peptide matches.", action="store_true"
)
protein_group.add_argument(
    "--case-sensitive",
...
mod_group.add_argument(
    "--mod-out",
    nargs="?",
    help="The file to write a modification-centric summary to.",
    type=argparse.FileType("w"),
    default=None,
)
mod_group.add_argument(
    "--modification-site",
    help="Write the position in the parent protein of the modification (requires case-sensitive and modifications being lower-cased).",
    action="store_true",
)
parser.add_column_function(
    "--mod-col", help="The column containing modification information.", group=mod_group
)
motif_group = mod_group.add_argument_group("Motif Options")
motif_group.add_argument(
    "--motifs",
    help="Enable creation of motifs for each modification.",
    action="store_true",
)
motif_group.add_argument(
    "--motif-window",
    help="The width of the motif window (how many residues to go out from each modification).",
    type=int,
    default=10,
)
motif_group.add_argument(
    "--motif-unique",
    help="Only output motifs where the peptide mapping is unambiguous.",
    action="store_true",
)
motif_group.add_argument(
    "--motif-out",
...
def mapper(peptides):
    ...
    for peptide, peptide_grouped in groups:
        ...
        matched[peptide] = {
            ...
            "positions": [
                group["peptide_start"]
                - protein_sequences[: group["peptide_start"]].rfind("\t")
                for group in pg
            ],
...
def main():
    ...
    if mod_site:
        ...
        inference = True
    precursor_columns = (
        [i for i in args.precursors.split(",")] if args.precursors else None
    )
    if ibaq:
        enzyme = digest.Enzyme(
            enzyme=args.enzyme[0] if isinstance(args.enzyme, list) else args.enzyme
        )
    sys.stderr.write("Reading in Fasta file.\n")
    fasta_headers, protein_sequences = zip(
        *[(header.replace(";", ""), sequence) for header, sequence in fasta_file]
    )
    ...
    if args.regex:
        ...
        sys.stderr.write(
            "{0} header sequences did not match regex {1} and have been discarded.\n".format(
                len(fasta_headers) - len(protein_sequences), args.regex
            )
        )
    if ibaq:
    ...
    with tsv_file as f:
        ...
        for line_num, entry in enumerate(reader):
            if (
            ...
            ):  # we assume the first header line is the one we care about
                ...
                if not precursor_columns:
                    precursor_columns = [
                        i for i, v in enumerate(entry) if "precursor" in v.lower()
                    ]
                try:
                ...
                if peptide not in peptide_history:
                    peptide_history[peptide] = {
                        "intensities": dict(
                            [
                                (i, set([]))
                                for i in six.moves.range(len(precursor_columns))
                            ]
                        )
                        if precursor_columns is not None
                        else {},
                    }
    ...
    subpeptides = [
        ...
        for n in six.moves.range(0, len(peptides), peptides_per_core)
    ]
    if n < len(peptides):
    ...
    for index, (peptide, d) in enumerate(peptide_history.iteritems()):
                        ...
                        for j, k in enumerate(peptide):
                            if k.islower():
                                ...
                                if mod_col or mod_site:
                                    try:
                                        mod_values = mod_grouping[protein][mod_key][
                                            "values"
                                        ]
                                        mod_peptides = mod_grouping[protein][mod_key][
                                            "peptides"
                                        ]
                                        if mod_col:
                                        ...
                                        mod_grouping[protein][mod_key][
                                            "values"
                                        ] = make_unique(mod_values)
                                        mod_grouping[protein][mod_key][
                                            "peptides"
                                        ] = make_unique(mod_peptides + [peptide])
                                    except KeyError:
                                        try:
                                            mod_grouping[protein][mod_key] = {
                                                "values": make_unique([d["mod_col"]])
                                                if mod_col
                                                else "",
                                                "peptides": make_unique([peptide]),
                                            }
                                        except KeyError:
        ...
        if ibaq:
            ...
            try:
                precursor_int = sum(
                    [
                        ...
                        for i in six.moves.range(len(normalizations))
                    ]
                )
            except decimal.InvalidOperation:
                precursor_int = 0
            ...
            for protein_index in mapped_info["accessions"]:
                            ...
                            set(
                                [
                                    ...
                                    for tryptic_peptides in [
                                        ...
                                        for possible_protein_index in mapped_info[
                                            "indices"
                                        ]
                                    ]
                ...
                ibaqs.append(
                    precursor_int / peptides if peptides and precursor_int else 0
                )
            peptide_dict["inference"]["iBAQ"] = (
                ibaq_col_func([int(IBAQ_NORMALIZATION * i) for i in ibaqs])
                if ibaqs
                else 0
            )
            entry.append(
        ...
        if out_position:
            entry.append(
                peptide_dict["inference"].get("matched_positions", "")
                if not unique or mapped_info["unique"]
                else ""
            )
        if mod_site:
            entry.append(
                peptide_dict["inference"].get("mod_sites", "")
                if not unique or mapped_info["unique"]
                else ""
            )
        if motif_search:
            entry.append(
                ";".join(
                    [
                        ...
                        for motif_site, motifs in peptide_dict["inference"]
                        .get("motifs", {})
                        .iteritems()
                    ]
                )
    ...
    if motif_search:
        with motif_out as o:
            ...
            for peptide, peptide_dict in peptide_grouping.iteritems():
                for motif_key, motifs in (
                    peptide_dict["inference"].get("motifs", {}).iteritems()
                ):
                    writer.writerow(
                        [
                            motif_key,
        ...
        with args.protein_out as o:
            ...
            for protein in protein_grouping:
                    ...
                    if not unique or mapped_peptides.get(peptide, {}).get("unique"):
                        ...
                        if ibaq and normalize:
                            try:
                                ...
                                pass
                entry.append(
                    ";".join(["%s(%s)" % (i, j) for i, j in peptide_psm_count])
                )
                ...
                if ibaq:
                    ...
                    peptides = cleaved.get(protein_index, None)
                    ibaq_value = [
                        int(IBAQ_NORMALIZATION * precursor_int / peptides)
                        if peptides and precursor_int
                        else 0
                    ]
                    ...
                    if d:
                        if inference:
                            entry.append(
                                ...
                                if not unique
                                or mapped_peptides.get(peptide, {}).get("unique")
                                else ""
                            )
                        if out_position:
                            entry.append(
                        ...
                        if mod_site:
                            ...
                            peptide_mods = {}
                            mod_entry = []
                            if not unique or mapped_peptides.get(peptide, {}).get(
                                "unique"
                            ):
                                for mod_protein in mod_proteins.split(";"):
                                    # mod protein looks like:
                                        ...
                                        for mod_prot_site in mod_prot_sites[:-1].split(
                                            ...
                                            if mod_prot_site:
                                                ...
                                                try:
                                                    ...
                                                    peptide_mods[mod_prot] = set(
                                                        [(mod_aa, mod_prot_site)]
                                                    )
                                                try:
                                                    mod_stats[mod_aa].add(
                                                        (mod_prot, mod_prot_site)
                                                    )
                                                except KeyError:
    ...
    mod_out = (
        args.mod_out
        if args.mod_out
        else open(os.path.join("{}_mods".format(out_file.name)), "wb")
    )
    with mod_out as o:
        writer = csv.writer(o, delimiter=delimiter)
